...
const EMPTY_RESULT: RenderResult = { session: [], activity: [], activityTitle: "" }
...
describe("renderLines structure", () => {
  ...
  test("inserts horizontal rule below header when body has content", () => {
    const result: RenderResult = { session: ["body line"], activity: [], activityTitle: "" }
  ...
  test("each row is left-padded with one space", () => {
    const result: RenderResult = { session: ["x"], activity: [], activityTitle: "" }
  ...
  test("activityTitle in header is dimmed", () => {
    const result: RenderResult = { session: [], activity: [], activityTitle: "ZZZ" }
...
describe("renderLines wrapping", () => {
  test("wraps a long body line at SEP boundaries when terminal is narrow", () => {
    ...
    const result: RenderResult = { session: [longLine], activity: [], activityTitle: "" }
    const lines = renderLines(MINIMAL_DATA, result).split("\n")
  ...
  test("does not wrap when line has no SEP even if it exceeds width", () => {
    ...
    const result: RenderResult = { session: [noSep], activity: [], activityTitle: "" }
    const lines = renderLines(MINIMAL_DATA, result).split("\n")
  ...
  test("rule width does not exceed usable width", () => {
    ...
    const result: RenderResult = { session: ["short"], activity: [], activityTitle: "" }
